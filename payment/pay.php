...
if (isset($_GET['f_no'])) {
    ...
    $authority = '';
    $sql="SELECT rate FROM `settings_rates` WHERE `to` between '$start' and '$end'";
    $rates= mysqli_query($conn,$sql);
    $row = $rates->fetch_assoc();
    $rate =$row["rate"];
    //(int) mysqli_query($rates,3,'rate');
    //echo $rate;
//$rates = mysqli_fetch_row($ratesrows);
    ...
    <div id="printable">
        <table id="receipt"  >
            ...
            <th colspan="2"  ><h1>Dairy Payment Receipt</h1></th>
            </thead>
            <tbody>
        ...
        echo '<thead class="" ><th>#</th><th>Date</th><th>Liters:</th></thead><tbody>';
        $count = 0;
        ...
        echo '</tbody></table>';
        echo '<h4>Total amount in Rs:'. ($farmer_total * $rate) .'</h4>';
        ?>
